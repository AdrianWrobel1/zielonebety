...
class StatsHubProvider(BaseProvider):
    ...
    def fetch(self, discovery_items: Optional[List[Any]] = None) -> List[Any]:
        ...
        if self.statshub_config.mode == "player_trends":
            ...
            # Multi-page player trends acquisition loop
            ...
            while current_page <= max_pages:
                try:
                    ...
                    raw_page = self.client.fetch_player_trends(
                        games=page_cfg.games,
                        player_id=page_cfg.player_id,
                        unique_tournament_id=page_cfg.unique_tournament_id,
                    ...
                    if isinstance(raw_page, dict):
                        items = raw_page.get("players") or raw_page.get("data") or raw_page.get("items") or []
                    ...
                    if collected_count >= max_results:
                        truncated = True
                        self.context.logger.info(f"StatsHub player trends reached max limit ({max_results}). Stopping pagination.")
                        break

                    current_page += 1
                except Exception as e:
                    self.context.logger.error(f"StatsHub player trends fetch failed on page {current_page}: {e}")
