"""
Player Prop Opportunity Ranking & Decision Engine (Stage 16 + Stage 29 Value Bet Engine)

Provides explainable, deterministic scoring, classification, actionability, and reasoning
for player proposition betting opportunities without black-box models.
...
Stage 29 Value Bet Engine Invariants:
- fair_odds = 1 / P_model
...
@dataclass
class PropOpportunityEvaluation:
    """Evaluation result from PropOpportunityEngine."""
    score: float  # 0.0 to 100.0
    classification: str  # "OPPORTUNITY", "SHORTLIST", "STANDARD", "SPECULATIVE"
    actionability: str  # "VALUEBET", "BETTABLE", "NO_EXECUTION_MARKET", "NO_EXECUTION_ODDS", "MATCH_UNCERTAIN", "REFERENCE_ONLY"
    historical_probability: float  # 0.0 to 1.0 (hits / sample_size)
    reference_market_probability: Optional[float]  # 1 / reference_odds
    market_probability: Optional[float]  # Alias for reference_market_probability for backward compatibility
    ...
    execution_edge_pct: Optional[float] = None  # execution_edge in percentage points (+60.8pp)
    ...
    # Stage 29 Value Bet Engine Fields
    ...
    is_valuebet: bool = False  # True iff positive EV on verified execution odds
    reasons: List[str] = field(default_factory=list)
    warnings: List[str] = field(default_factory=list)
    data_quality_flags: List[str] = field(default_factory=list)
    score_breakdown: Dict[str, float] = field(default_factory=dict)
    edge_type: str = "STATISTICAL_EDGE"  # Explicitly distinguished from PROVEN_FAIR_VALUE_EDGE
...
class PropOpportunityEngine:
    """Deterministic Player Prop Opportunity Engine, Value Bet Engine, and Decision Layer."""
    ...
    def evaluate(
        ...
        line: float,
        side: str = "OVER",
        best_odds: Optional[float] = None,  # Reference odds (Bet365, etc.)
        best_bookmaker: Optional[str] = None,
        best_execution_odds: Optional[float] = None,  # Polish execution odds (Superbet, Betclic)
    ...
    ) -> PropOpportunityEvaluation:
        """Evaluate, score, and rank a player prop opportunity with Value Bet Engine detection."""
        reasons: List[str] = []
        warnings: List[str] = []
        ...
        # INVARIANT: Only calculate execution edge when execution_status is BETTABLE and quote is valid!
        ...
        if is_bettable_execution:
            ...
            if has_sample:
                ...
                # Determine Valuebet qualification: positive EV on real Polish execution price
        ...
        if side.upper() == "OVER":
            if stat_average > line_val:
                ...
                reasons.append(f"Average {stat_average:.2f} {stat_name} exceeds line {line_val:.1f} (+{diff:.2f} margin)")
            elif stat_average > 0:
                diff = line_val - stat_average
                warnings.append(f"Average {stat_average:.2f} {stat_name} is below line {line_val:.1f} (-{diff:.2f})")

            if last_5_avg is not None:
                if last_5_avg > line_val:
                    reasons.append(f"Strong recent form: Last 5 games avg {last_5_avg:.1f} vs line {line_val:.1f}")
                elif last_5_avg < line_val * 0.7:
                    warnings.append(f"Cold recent form: Last 5 games avg {last_5_avg:.1f} is below line {line_val:.1f}")

            if last_10_avg is not None and last_10_avg > line_val:
                reasons.append(f"Sustained form: Last 10 games avg {last_10_avg:.1f} vs line {line_val:.1f}")

        # 5. Sample Size Reliability
        if effective_sample < self.MIN_SAMPLE_FOR_OPPORTUNITY:
            warnings.append(f"Low sample size ({effective_sample} matches). Statistical reliability is limited.")
        elif effective_sample >= 10:
            reasons.append(f"Reliable sample size ({effective_sample} matches evaluated)")
        ...
        # Dampen score for small sample sizes to avoid treating tiny samples as statistically certain
        if effective_sample < self.MIN_SAMPLE_FOR_OPPORTUNITY:
            max_small_sample_score = 45.0 + (effective_sample * 5.0)  # N=2 -> max 55, N=4 -> max 65
            total_score = max(0.0, min(max_small_sample_score, raw_total))
        else:
            total_score = max(0.0, min(100.0, raw_total))
        total_score = round(total_score, 1)

        # 7. Opportunity Classification (Statistical Quality)
        ...
        # 8. Actionability State & Distinct Status (Real Execution Status)
        # CRITICAL INVARIANT:
        # - VALUEBET status: Real execution odds exist + EV > 0 (and >= min_ev_threshold)
        # - BETTABLE status: Real execution odds exist + status is BETTABLE (but EV <= 0)
        # - REFERENCE_ONLY / NO_EXECUTION_MARKET / NO_EXECUTION_ODDS / MATCH_UNCERTAIN
        if is_bettable_execution:
        ...
        return PropOpportunityEvaluation(
            score=total_score,
